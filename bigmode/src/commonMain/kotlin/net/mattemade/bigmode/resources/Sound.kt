...
class Sound(private val id: String, private val entries: List<ResourceSound>, private val gameContext: BigmodeGameContext) {
    ...
    private fun getNextEntry(): Pair<ResourceSound, AudioClipEx> {
        val entry = pool[currentIndex].entry
        currentIndex = (currentIndex + 1) % pool.size
        return entry
    }

    fun clip(): Pair<ResourceSound, AudioClipEx> =
        getNextEntry()
