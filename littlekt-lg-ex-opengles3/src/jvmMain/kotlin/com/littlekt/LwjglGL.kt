...
class LwjglGL(private val engineStats: EngineStats, private val graphics: Graphics, private val hdpiMode: HdpiMode) :
    ...
    override fun getActiveAttrib(
        ...
        return glGetActiveAttrib(glShaderProgram.address, index, 256, size.buffer, type.buffer)
    }
    ...
    override fun getActiveUniform(
        ...
        size as IntBufferImpl
        type as IntBufferImpl
        return glGetActiveUniform(glShaderProgram.address, index, 256, size.buffer, type.buffer)
    }
    ...
    override fun getProgramiv(glShaderProgram: GlShaderProgram, pname: Int, params: IntBuffer) {
        ...
        return glGetProgramiv(glShaderProgram.address, pname, params.buffer)
    }
    ...
    override fun bufferSubData(target: Int, offset: Int, data: Buffer) {
        ...
        when (data) {
            is FloatBuffer -> {
                data as FloatBufferImpl
                glBufferSubData(target, offset.toLong(), data.buffer)
            }
    ...
    override fun drawElementsInstanced(
        ...
        engineStats.drawCalls++
        engineStats.vertices += count
        glDrawElementsInstanced(mode, count, type, offset.toLong(), instanceCount)
    }
    ...
    override fun compressedTexImage2D(
        ...
        if (source != null) {
            ...
            glCompressedTexImage2D(target, level, internalFormat, width, height, 0, source.buffer)
        } else {
            glCompressedTexImage2D(target, level, internalFormat, width, height, 0, null)
        }
    }
    ...
    override fun compressedTexSubImage2D(
        ...
        engineStats.calls++
        source as ByteBufferImpl
        glCompressedTexSubImage2D(target, level, xOffset, yOffset, width, height, format, source.buffer)
    }
    ...
    override fun texSubImage2D(
        ...
        engineStats.calls++
        source as ByteBufferImpl
        glTexSubImage2D(target, level, xOffset, yOffset, width, height, format, type, source.buffer)
    }
    ...
    override fun compressedTexImage3D(
        ...
        if (source != null) {
            source as ByteBufferImpl
            glCompressedTexImage3D(target, level, internalFormat, width, height, depth, 0, source.buffer)
        } else {
            glCompressedTexImage3D(target, level, internalFormat, width, height, depth, 0, null)
        }
    }
    ...
    override fun readPixels(x: Int, y: Int, width: Int, height: Int, format: Int, type: Int, pixels: ByteBuffer) {
        glReadPixels(x, y, width, height, format, type, (pixels as? ByteBufferImpl)?.buffer!!)
    }
}