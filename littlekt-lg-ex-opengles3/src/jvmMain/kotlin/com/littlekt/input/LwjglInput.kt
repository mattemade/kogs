...
class LwjglInput(private val context: LwjglContext, ) : Input {
    ...
    private val _activeInputProcessors = mutableListOf<InputProcessor>()
    private val inputCache = InputCache(_activeInputProcessors)
    ...
    fun attachToWindow(windowHandle: Long) {
        ...
        glfwSetCursorPosCallback(windowHandle) { _, xpos, ypos ->
            ...
            inputCache.onMove(mouseX, mouseY, _deltaX, _deltaY, touchedPointers.lastOrNull() ?: Pointer.POINTER1)
        }

        glfwSetMouseButtonCallback(windowHandle) { _, button, action, _ ->
            if (action == GLFW_PRESS) {
                inputCache.onTouchDown(mouseX, mouseY, _deltaX, _deltaY, button.getPointer)
                touchedPointers += button.getPointer
            } else {
                inputCache.onTouchUp(mouseX, mouseY, _deltaX, _deltaY, button.getPointer)
                touchedPointers -= button.getPointer
            }
        }
    ...
    override fun addActiveInputProcessor(processor: InputProcessor) {
        _activeInputProcessors += processor
    }
    ...
    override fun removeActiveInputProcessor(processor: InputProcessor) {
        _activeInputProcessors -= processor
    }
