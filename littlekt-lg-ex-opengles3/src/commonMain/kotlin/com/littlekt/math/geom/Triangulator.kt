 ...
 * garbage out).
 *
 * If the polygon vertices are very large or very close together then [isClockwise] may not
 ...
 *
 * Ported from: [EarClippingTriangulator](https://github.com/libgdx/libgdx/blob/master/gdx/src/com/badlogic/gdx/math/EarClippingTriangulator.java)
 * @author Colton Daily
 * @date 7/19/2022
 */
class Triangulator {
    ...
    private val triangles = ShortArrayList(6)

    /**
     * Triangulates the given (convex or concave) simple polygon to a list of triangle vertices.
     * @param vertices pairs describing vertices of the polygon, in either clockwise or counterclockwise order.
     * @return triples of triangle indices in clockwise order. Note the returned array is reused for later calls to the
     * same method.
     */
    fun computeTriangles(vertices: FloatArrayList) = computeTriangles(vertices.data, 0, vertices.size)
    ...
    private val Int.previousIndex: Int get() = (if (this == 0) vertexCount else this) - 1
    private val Int.nextIndex: Int get() = (this + 1) % vertexCount
