...
open class Vec2f internal constructor(x: Float, y: Float, size: Int) {
    ...
    fun distance(otherX: Float, otherY: Float): Float = sqrt(sqrDistance(otherX, otherY))

    fun dot(other: Vec2f): Float = x * other.x + y * other.y

    /**
     * Checks vector components for equality using [com.littlekt.math.isFuzzyEqual], that is all components must
    ...
    fun sqrDistance(otherX: Float, otherY: Float): Float {
        val dx = x - otherX
        val dy = y - otherY
        return dx * dx + dy * dy
    }
