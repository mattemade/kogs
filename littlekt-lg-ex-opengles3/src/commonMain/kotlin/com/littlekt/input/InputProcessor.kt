...
interface InputProcessor {
     ...
     * @return true if event is handled; false otherwise
     */
    fun touchDragged(screenX: Float, screenY: Float, movementX: Float, movementY: Float, pointer: Pointer): Boolean = false
...
class InputProcessBuilder {
    ...
    private val touchDragged = mutableListOf<(Float, Float, Float, Float, Pointer) -> Boolean>()
    ...
    fun onTouchDraggedHandle(action: (screenX: Float, screenY: Float, movementX: Float, movementY: Float, pointer: Pointer) -> Boolean) {
        touchDragged += action
    }

    fun onTouchDragged(action: (screenX: Float, screenY: Float, movementX: Float, movementY: Float, pointer: Pointer) -> Unit) {
        touchDragged += { screenX, screenY, movementX, movementY, pointer ->
            action(screenX, screenY, movementX, movementY, pointer)
            false
        }
    }
    ...
    fun build() = object : InputProcessor {
        ...
        override fun touchDragged(screenX: Float, screenY: Float, movementX: Float, movementY: Float, pointer: Pointer): Boolean {
            var handled = false
            touchDragged.fastForEach { if (it.invoke(screenX, screenY, movementX, movementY, pointer)) handled = true }
            return handled
        }
