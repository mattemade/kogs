...
import com.littlekt.graphics.shader.shaders.DefaultFragmentShader
import com.littlekt.graphics.shader.shaders.DefaultVertexShader
import com.littlekt.graphics.textureMesh
...
class SpriteBatch(
    ...
    override fun draw(
        ...
        if (offset != null) {
            ...
            x1 += offset.getOrElse(index++) { 0f }
            y1 += offset.getOrElse(index++) { 0f }
            x2 += offset.getOrElse(index++) { 0f }
            y2 += offset.getOrElse(index++) { 0f }
            x3 += offset.getOrElse(index++) { 0f }
            y3 += offset.getOrElse(index++) { 0f }
            x4 += offset.getOrElse(index++) { 0f }
            y4 += offset.getOrElse(index) { 0f }
        }

        if (cyclingOffset != null) {
            var index = 0
            val size = cyclingOffset.size

            x1 += cyclingOffset[index++ % size]
            y1 += cyclingOffset[index++ % size]
            x2 += cyclingOffset[index++ % size]
            y2 += cyclingOffset[index++ % size]
            x3 += cyclingOffset[index++ % size]
            y3 += cyclingOffset[index++ % size]
            x4 += cyclingOffset[index++ % size]
            y4 += cyclingOffset[index % size]
