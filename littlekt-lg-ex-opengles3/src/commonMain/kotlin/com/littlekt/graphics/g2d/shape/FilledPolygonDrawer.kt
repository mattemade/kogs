...
internal class FilledPolygonDrawer(batchManager: BatchManager) : Drawer(batchManager) {
    ...
    fun rectangle(
        ...
        var index = 0
        val size = offset?.size ?: 0
        x1(halfWidth * cos - halfHeight * sin + centerX + (offset?.getOrNull(index++ % size) ?: 0f))
        y1(halfWidth * sin + halfHeight * cos + centerY + (offset?.getOrNull(index++ % size) ?: 0f))
        x2(-halfWidth * cos - halfHeight * sin + centerX + (offset?.getOrNull(index++ % size) ?: 0f))
        y2(-halfWidth * sin + halfHeight * cos + centerY + (offset?.getOrNull(index++ % size) ?: 0f))
        x3(-halfWidth * cos - (-halfHeight * sin) + centerX + (offset?.getOrNull(index++ % size) ?: 0f))
        y3(-halfWidth * sin + (-halfHeight * cos) + centerY + (offset?.getOrNull(index++ % size) ?: 0f))
        x4(halfWidth * cos - (-halfHeight * sin) + centerX + (offset?.getOrNull(index++ % size) ?: 0f))
        y4(halfWidth * sin + (-halfHeight * cos) + centerY + (offset?.getOrNull(index % size) ?: 0f))
        color(c1, c2, c3, c4)
    ...
    fun triangle(x1: Float, y1: Float, x2: Float, y2: Float, x3: Float, y3: Float, c1: Float, c2: Float, c3: Float) {
        val caching = batchManager.cachingDraws
        vert1(x1, y1)
        vert2(x2, y2)
        vert3(x3, y3)
    ...
    fun polygon(vertices: FloatArray, offset: Int = 0, count: Int = vertices.size) {
        ...
        polygon(vertices, triangles.data, triangles.size)
    }
    ...
    fun polygon(vertices: FloatArray, triangles: ShortArray, trianglesCount: Int = triangles.size / 3) {
        val c = batchManager.colorBits
        for (i in 0 until trianglesCount) {
            batchManager.ensureSpaceForTriangle()
